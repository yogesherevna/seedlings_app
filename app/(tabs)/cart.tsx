...
export default function Cart() {
 ...
 <View style={{flexDirection:'row',alignItems:'center',marginTop:10,gap:12}}><Pressable onPress={()=>change(i.id,-1)}><Text style={styles.step}>−</Text></Pressable><Text style={{fontWeight:'900'}}>{i.quantity}</Text><Pressable onPress={()=>change(i.id,1)}><Text style={styles.step}>+</Text></Pressable><Pressable onPress={()=>remove(i.id)} style={{marginLeft:'auto'}}><Text style={{color:colors.danger,fontWeight:'800'}}>Remove</Text></Pressable></View></View></View>)}
 ...
 <Button title="Proceed to Checkout" onPress={()=>router.push('/checkout')}/></View></>}
 </Screen>
}
