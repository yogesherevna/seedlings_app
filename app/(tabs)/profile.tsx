...
const items=[['My Profile','/account/profile'],['Delivery Addresses','/account/addresses'],['My Orders','/(tabs)/orders'],['My Subscriptions','/account/subscriptions'],['My Wallet','/account/wallet'],['Notifications','/account/notifications'],['Feedback','/account/feedback'],['About','/account/about']];
export default function Profile(){const mobile=useAppStore(s=>s.mobile); const logout=useAppStore(s=>s.logout); return <Screen>
 ...
 <Pressable onPress={()=>{logout();router.replace('/auth/login')}} style={{padding:16,alignItems:'center'}}><Text style={{color:colors.danger,fontWeight:'900'}}>Logout</Text></Pressable>
 </Screen>}
