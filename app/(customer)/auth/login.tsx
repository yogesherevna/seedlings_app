  ...
  return <Screen scroll={false}><View style={{alignItems:'center',paddingTop:38}}>
    ...
    <View style={{width:'100%'}}>
      <View style={{flexDirection:'row',gap:8,marginBottom:12}}>
        <View style={{width:72,backgroundColor:colors.panel,borderWidth:1,borderColor:colors.line,borderRadius:10,justifyContent:'center',alignItems:'center'}}><Text style={{fontSize:15}}>🇮🇳 +91</Text></View>
        <View style={{flex:1}}><TextInput value={mobile} onChangeText={v=>setMobile(v.replace(/\D/g,''))} keyboardType="phone-pad" maxLength={10} placeholder="9876543210" placeholderTextColor={colors.inkFaint} style={{height:50,backgroundColor:colors.panel,borderWidth:1,borderColor:colors.line,borderRadius:10,paddingHorizontal:14,fontSize:16,color:colors.ink}} /></View>
      </View>
      <Button title="Send OTP" onPress={submit}/>
      <View style={{height:10}}/><Button title="Continue as Guest" secondary onPress={()=>router.replace('/(customer)/(tabs)')}/>
    </View>
  </View></Screen>;
}
