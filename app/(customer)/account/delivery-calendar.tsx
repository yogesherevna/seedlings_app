...
export default function DeliveryCalendar() {
  ...
  return <Screen>
    ...
    {loading ? <View style={{ paddingVertical: 60, alignItems: 'center' }}><ActivityIndicator size="large" color={colors.green} /><Text style={{ color: colors.inkSoft, marginTop: 12 }}>Loading delivery calendar…</Text></View> : error ? <View style={{ backgroundColor: colors.panel, borderRadius: 14, padding: 16 }}><Text style={{ color: colors.danger, fontWeight: '800' }}>{error}</Text></View> : !active ? <View style={{ backgroundColor: colors.panel, borderRadius: 14, padding: 16 }}><Text style={{ fontSize: 17, fontWeight: '900', color: colors.ink }}>No active subscription</Text><Text style={{ color: colors.inkSoft, marginTop: 7 }}>Your delivery calendar will appear here when you have an active subscription.</Text></View> : <>
      <View style={{ backgroundColor: colors.panel, borderRadius: 15, borderWidth: 1, borderColor: colors.lineSoft, padding: 16, marginBottom: 14 }}>
      ...
      {dates.length ? dates.map((key, index) => {
        ...
        return <View key={key} style={{ backgroundColor: colors.panel, borderRadius: 13, borderWidth: 1, borderColor: isNext ? colors.green : colors.lineSoft, padding: 14, marginBottom: 9 }}>
