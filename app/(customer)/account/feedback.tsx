...
export default function FeedbackScreen() {
  ...
  return <Screen>
    ...
    <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void load(true)} />} contentContainerStyle={{ paddingBottom: 30 }}>
      <View style={{ backgroundColor: colors.panel, borderRadius: 15, padding: 16, borderWidth: 1, borderColor: colors.lineSoft }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>{[1,2,3,4,5].map((n) => <Pressable key={n} onPress={() => setRating(n)} accessibilityLabel={`${n} star rating`}><Text style={{ fontSize: 32, color: n <= rating ? colors.orange : colors.line }}>★</Text></Pressable>)}</View>
        ...
        <TextInput value={comment} onChangeText={setComment} multiline maxLength={1000} placeholder="Write your feedback" placeholderTextColor={colors.inkSoft} style={{ minHeight: 110, textAlignVertical: 'top', backgroundColor: colors.greenTint, borderWidth: 1, borderColor: colors.lineSoft, borderRadius: 12, padding: 12, marginTop: 10, color: colors.ink }} />
        {!!error && <Text style={{ color: colors.danger, marginTop: 10 }}>{error}</Text>}
        <Pressable disabled={saving} onPress={() => void submit()} style={{ backgroundColor: saving ? colors.line : colors.green, borderRadius: 12, padding: 15, alignItems: 'center', marginTop: 12 }}><Text style={{ color: colors.white, fontWeight: '900' }}>{saving ? 'Submitting…' : 'Submit Feedback'}</Text></Pressable>
      </View>

      <Text style={{ fontSize: 18, fontWeight: '900', color: colors.ink, marginTop: 20, marginBottom: 10 }}>Your previous feedback</Text>
      {loading ? <View style={{ padding: 30, alignItems: 'center' }}><ActivityIndicator /></View> : items.length === 0 ? <View style={{ backgroundColor: colors.panel, borderRadius: 14, padding: 16 }}><Text style={{ fontWeight: '900', color: colors.ink }}>No feedback submitted yet</Text></View> : items.map((item) => <View key={item.id} style={{ backgroundColor: colors.panel, borderRadius: 14, padding: 15, marginBottom: 10 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between' }}><Text style={{ color: colors.orange, fontSize: 18 }}>{'★'.repeat(Math.max(0, Math.min(5, item.rating)))}{'☆'.repeat(Math.max(0, 5 - Math.min(5, item.rating)))}</Text><Text style={{ color: colors.inkSoft, fontSize: 11 }}>{dateText(item.createdAt)}</Text></View>{!!item.comment && <Text style={{ color: colors.ink, lineHeight: 20, marginTop: 8 }}>{item.comment}</Text>}{item.status && <Text style={{ color: colors.inkSoft, fontSize: 11, marginTop: 7 }}>Status: {item.status}</Text>}</View>)}
