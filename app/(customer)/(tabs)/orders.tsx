...
export default function Orders() {
  ...
  return <Screen>
    ...
    {loading ? <View style={{ paddingTop: 35, alignItems: 'center' }}><ActivityIndicator size="large" color={colors.green} /><Text style={{ color: colors.inkSoft, marginTop: 10 }}>Loading your orders…</Text></View>
      : error ? <View style={{ backgroundColor: colors.panel, borderRadius: 14, borderWidth: 1, borderColor: colors.lineSoft, padding: 16 }}><Text style={{ fontWeight: '800', color: colors.ink }}>{error}</Text><Pressable onPress={load} style={{ marginTop: 12 }}><Text style={{ color: colors.greenDark, fontWeight: '900' }}>Try again</Text></Pressable></View>
      : filtered.length === 0 ? <View style={{ backgroundColor: colors.panel, borderRadius: 14, borderWidth: 1, borderColor: colors.lineSoft, padding: 20, marginTop: 5 }}><Text style={{ fontWeight: '900', fontSize: 17, color: colors.ink }}>{orders.length ? 'No matching orders' : 'No orders yet'}</Text><Text style={{ color: colors.inkSoft, marginTop: 6, lineHeight: 20 }}>{orders.length ? 'Try another order filter.' : 'Your one-time purchases and subscription orders will appear here.'}</Text><Pressable onPress={() => router.push('/(customer)/(tabs)/products')} style={{ marginTop: 14 }}><Text style={{ color: colors.greenDark, fontWeight: '900' }}>Shop Fresh →</Text></Pressable></View>
      : filtered.map((o) => {
        ...
        return <Pressable key={o.id} onPress={() => router.push(`/(customer)/order/${o.id}`)} style={{ backgroundColor: colors.panel, borderRadius: 14, borderWidth: 1, borderColor: colors.lineSoft, padding: 15, marginBottom: 10 }}>
